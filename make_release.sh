...
ASPRELOAD_SRC=latency_reduction/src/as_preload.so
ASPRELOAD_DES=Koriki/lib/as_preload.so
...
for binary in "${SM_SRC};${SM_DES}" "${INVOKER_SRC};${INVOKER_DES}" "${RA_SRC};${RA_DES}" "${ASPRELOAD_SRC};${ASPRELOAD_DES}" "${COMMANDER_SRC};${COMMANDER_DES}" "${GMU_SRC};${GMU_DES}" "${BOOTSCRENSEL_SRC};${BOOTSCRENSEL_DES}" "${SYSINFO_SRC};${SYSINFO_DES}" "${CHARGING_SRC};${CHARGING_DES}" "${KEYMON_SRC};${KEYMON_DES}" "${SHOWSCREEN_SRC};${SHOWSCREEN_DES}"
do
...
for bin_destiny in "${SM_DES}" "${INVOKER_DES}" "${RA_DES}" "${ASPRELOAD_DES}" "${COMMANDER_DES}" "${GMU_DES}" "${BOOTSCRENSEL_DES}" "${SYSINFO_DES}" "${CHARGING_DES}" "${KEYMON_DES}" "${SHOWSCREEN_DES}"
do
    rm ${bin_destiny}
done