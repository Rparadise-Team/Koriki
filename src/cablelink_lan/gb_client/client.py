            ...
            if event.type == KEYDOWN:
                if event.key == K_UP:
                    ...
                    if selected_roms[0] is None:
                        ...
                        pygame.quit()
                        time.sleep(1)
                        SU.call(['/mnt/SDCARD/App/gb_client/tgbdual_libretro_client', rom1_path, rom2_path])
                        quit()
                        return
...
if __name__ == "__main__":
    ...
    if not ip_exists:
        show_message("GB/GBC ROMs Selector", "Press MENU for exit", "You must connect to an adhoc network how client")
        while True:
            for event in pygame.event.get():
